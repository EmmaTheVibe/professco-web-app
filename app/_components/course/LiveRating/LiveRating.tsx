...
export default function LiveRating({ currentRating, setCurrentRating }: Props) {
  const [hoverRating, setHoverRating] = useState(0);

  const handleMouseMove = (event: React.MouseEvent<HTMLImageElement>, starValue: number) => {
  ...
  const handleClick = (event: React.MouseEvent<HTMLImageElement>, starValue: number) => {
    const starRect = event.currentTarget.getBoundingClientRect();
    const x = event.clientX - starRect.left;
    const isHalf = x < starRect.width / 2;
